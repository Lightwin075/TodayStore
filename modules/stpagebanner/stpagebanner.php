...
class StPageBanner extends Module implements WidgetInterface
{
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
					$result['error'][] = $this->getTranslator()->trans('An error occurred during move image.', array(), 'Admin.Theme.Panda');
				else{
	...
	{        
		...
		$this->fields_form_banner[0]['form'] = array(
			...
			'input' => array(
                'links' => array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Page:', array(), 'Admin.Theme.Panda'),
                    'name' => 'links',
                ...
                array(
                    ...
                    'suffix' => 'px',
                    'class' => 'fixed-width-lg',
                    'desc' => $this->getTranslator()->trans('Leave it empty to use the default value 12.', array(), 'Admin.Theme.Panda'),
                ),
                /*
                array(
                    'type' => 'switch',
        ...
        $this->fields_form_banner[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Top border height:', array(), 'Admin.Theme.Panda'),
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Bottom border height:', array(), 'Admin.Theme.Panda'),
                    'name' => 'border_size',
                    'prefix' => 'px',
                    'class' => 'fixed-width-lg',
                    'default_value' => '',
                    'validation' => 'isNullOrUnsignedId',
                    'desc' => $this->getTranslator()->trans('Leave it empty to use the default value 1.', array(), 'Admin.Theme.Panda'),
                ),
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Do not show breadcrumb:', array(), 'Admin.Theme.Panda'),
    ...
    {
        return array(
            'authentication' => array('id'=>'10_authentication', 'name'=>$this->getTranslator()->trans('Login', array(), 'Admin.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Login', array(), 'Admin.Theme.Panda')),
            'my-account' => array('id'=>'10_my-account', 'name'=>$this->getTranslator()->trans('My account', array(), 'Admin.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Manage my customer account', array(), 'Modules.StPagebanner.Admin')),
            ...
            'order-follow' => array('id'=>'10_order-follow', 'name'=>$this->getTranslator()->trans('My merchandise returns', array(), 'Admin.Theme.Panda'), 'title'=>$this->getTranslator()->trans('My returns', array(), 'Modules.StPagebanner.Admin')),
            ...
            'identity' => array('id'=>'10_identity', 'name'=>$this->getTranslator()->trans('My personal info', array(), 'Admin.Theme.Panda'), 'title'=>$this->getTranslator()->trans('Manage my personal information', array(), 'Modules.StPagebanner.Admin')),
    ...
    public static function displayTitle($value, $row)
    {
        ...
        switch($row['item_k'])
        {
            case 0:
                ...
                return $module->getTranslator()->trans('All', array(), 'Admin.Theme.Panda');
            break;
            case 1:
            ...
            case 8:
                if(Module::isInstalled('stblog') && Module::isEnabled('stblog'))
                {
                    ...
                    if(Validate::isLoadedObject($category))
                        ...
                        {
                            $module = new StPageBanner();
                            return $module->getTranslator()->trans('Blog', array(), 'Admin.Theme.Panda');
                        }
                        else
                            return $category->name;
                }
                ...
                if (strpos($row['item_v'],'_'))
                {
                    ...
                    if ($type && $id)
                    {
                        if ($type == 1)
                        {
                            ...
                            if(Validate::isLoadedObject($category))
                                return $category->name.'('.$module->getTranslator()->trans('Category', array(), 'Admin.Theme.Panda').')';
                        }
                        if ($type == 4)
                        {
                            $manufacturer = new Manufacturer((int)$id, (int)$id_lang);
                            if ($manufacturer->id)
                                return $manufacturer->name.'('.$module->getTranslator()->trans('Manufacturer', array(), 'Admin.Theme.Panda').')';
                        }
                    }    
                }
                return $module->getTranslator()->trans('All', array(), 'Admin.Theme.Panda');
	...
	protected function initList()
	{
		...
		$this->fields_list = array(
            ...
            'item_v' => array(
                'title' => $this->getTranslator()->trans('Page', array(), 'Admin.Theme.Panda'),
                'type' => 'text',
