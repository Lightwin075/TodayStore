...
<section id="st_blog_related_article{$smarty.capture.column_slider}" class="block {if isset($column_slider) && $column_slider} column_block {/if} {if $hide_mob} hidden-xs {/if}">
    <h3 class="title_block {if (!isset($column_slider) || !$column_slider) && $title_position} title_block_center {/if}"><span>{l s='Related articles ' d='Shop.Theme.Panda'}</span></h3>
        ...
        {foreach $blogs as $blog}
            <div class="block_blog {if $blog@first}first_item{elseif $blog@last}last_item{/if} ">
                ...
                <div class="blog_read_more">
                    <a href="{$blog.link|escape:'html'}" title="{$blog.name|escape:'htmlall':'UTF-8'}" class="btn btn-default">{l s='Read more' d='Shop.Theme.Panda'}</a>
                </div>
            </div>
        {/foreach}
        </div>
