...
class StBlogCommentsMyCommentsModuleFrontController extends StblogModuleFrontController
{
		...
		if ($this->context->customer->isLogged())
		{
			if (Tools::isSubmit('submitAvatar'))
			{
				...
				if (!count($errors))
				{
                    ...
                    if (true === $rs)
                        $message = $this->trans('Avatar uploaded successfully', array(), 'Shop.Theme.Panda');
					elseif(false === $rs)
                        $errors[] = $this->trans('Failed to upload avatar', array(), 'Shop.Theme.Panda');
                    elseif (-1 === $rs)
                        $errors[] = $this->trans('This image is too large to be uploaded.', array(), 'Shop.Theme.Panda');
                    elseif(-2 === $rs)
                        $errors[] = $this->trans('Unable to upload this image, upload folder is not writable.', array(), 'Shop.Theme.Panda');
				}
			}
            elseif(Tools::getValue('act') == 'delavatar')
            {
                ...
                {
                    $message = $this->trans('Avatar deleted successfully', array(), 'Shop.Theme.Panda');
                    Tools::redirect($this->context->link->getModuleLink('stblogcomments','mycomments'));
                }
                else
                    $errors[] = $this->trans('Failed to delete avatar', array(), 'Shop.Theme.Panda');
            }
		}
		else
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = array(
            'title' => $this->trans('My blog comments', array(), 'Shop.Theme.Panda'),
