...
class StBlogCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	protected function ajaxProcessAddComment()
	{
		...
		if (!Validate::isInt(Tools::getValue('id_st_blog')))
			$errors[] = $this->trans('Invalidate blog ID ', array(), 'Shop.Theme.Panda');
		...
		if (!$id_customer && Tools::getValue('customer_email') && !Validate::isEmail(Tools::getValue('customer_email')))
			$errors[] = $this->trans('Customer email is invalidate', array(), 'Shop.Theme.Panda');
		...
		if (!$blog->id)
			$errors[] = $this->trans('Blog not found', array(), 'Shop.Theme.Panda');
		...
		if (!count($errors))
		{
			...
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + Configuration::get('ST_BLOG_C_MINIMAL_TIME')) < time()))
			{
                ...
                $result['moderation'] = $this->trans('Your comment may be awaiting moderation before being published.', array(), 'Shop.Theme.Panda');
			}
			else
			{
                $result['m'] = $this->trans('You should wait %seconds% seconds before posting a new comment.', array('%seconds%'=>Configuration::get('ST_BLOG_C_MINIMAL_TIME')), 'Shop.Theme.Panda');
			}
		}
		else
