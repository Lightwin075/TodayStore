...
<section id="st_blog_latest_comments" class="block column_block">
    <div class="title_block flex_container title_align_0 title_style_{(int)$stblog.heading_style}">
        ...
        <div class="title_block_inner">{l s='Latest Comments' d='Shop.Theme.Panda'}</div>
		...
		<ul class="pro_column_list base_list_line medium_list">
            ...
            <li class="clearfix line_item pro_column_box">
    			...
    			<div class="pro_column_right">
                    ...
                    {l s='on' d='Shop.Theme.Panda'} <a href="{$link->getModuleLink('stblog', 'article',['id_st_blog'=>$comment['id_st_blog'],'rewrite'=>$comment['link_rewrite']])}" title="{$comment.name}">{$comment.name|truncate:50:'...'}</a>
                </div>
            </li>
            {/foreach}
        </ul>
