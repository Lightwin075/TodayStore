...
class StNewsLetter extends Module implements WidgetInterface
{
    ...
    private function initPages()
    {
        $this->_pages = array(
                ...
                array(
                    'id' => 'order',
                    'val' => '512',
                    'name' => $this->getTranslator()->trans('Shopping cart', array(), 'Admin.Theme.Panda')
                ),
            );
    }
            ...
            if (!count($error) && $news_letter->validateFields(false) && $news_letter->validateFieldsLang(false))
            {
                ...
                if($news_letter->save())
                {
                    ...
                    if(isset($_POST['savestnewsletterAndStay']) || Tools::getValue('fr') == 'view')
                    {
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Newsletter', array(), 'Admin.Theme.Panda').' '.($id_st_news_letter ? $this->getTranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                }                    
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during newsletter', array(), 'Modules.StNewsletter.Admin').' '.($id_st_news_letter ? $this->getTranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
    ...
    {        
        ...
        $this->fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Configuration', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
            ),
            'input' => array(
                'location' => array(
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
						...
						'desc' => $this->getTranslator()->trans('Leave blank to disable by default.', array(), 'Admin.Theme.Panda'),
                        'validation' => 'isString',
					),
            ),
			'submit' => array(
    ...
    */
	...
	public function newsletterRegistration()
	{
		...
		if (empty($_POST['email']) || !Validate::isEmail($_POST['email']))
			$error = $this->getTranslator()->trans('Invalid email address.', array(), 'Modules.StNewsletter.Admin');
		/* Unsubscription */
		else if ($_POST['action'] == '1')
		{
			...
			if ($register_status < 1)
				$error = $this->getTranslator()->trans('This email address is not registered.', array(), 'Modules.StNewsletter.Admin');
			elseif (!$newsletterClass->unregister($_POST['email'], $register_status))
				$error = $this->getTranslator()->trans('An error occurred while attempting to unsubscribe.', array(), 'Modules.StNewsletter.Admin');
			else
                $valid = $this->getTranslator()->trans('Unsubscription successful.', array(), 'Modules.StNewsletter.Admin');
		}
		/* Subscription */
		else if ($_POST['action'] == '0')
		{
            ...
            if ($register_status > 0)
                $error = $this->getTranslator()->trans('This email address is already registered.', array(), 'Modules.StNewsletter.Admin');
			...
			if (!$error && !$newsletterClass->isRegistered($register_status))
			{
				if (Configuration::get('ST_NW_VERIFICATION_EMAIL'))
				{
					...
					if (!$token = $newsletterClass->getToken($email, $register_status)) {
					   $error = $this->getTranslator()->trans('An error occurred during the subscription process.', array(), 'Modules.StNewsletter.Admin');
					}
                    
                    $this->sendVerificationEmail($email, $token);

					$valid = $this->getTranslator()->trans('A verification email has been sent. Please check your inbox.', array(), 'Modules.StNewsletter.Admin');
				}
				else
				{
					if ($newsletterClass->register($email, $register_status))
						$valid = $this->getTranslator()->trans('You have successfully subscribed to this newsletter.', array(), 'Modules.StNewsletter.Admin');
					else
						$error = $this->getTranslator()->trans('An error occurred during the subscription process.', array(), 'Modules.StNewsletter.Admin');
	...
	public function confirmEmail($token)
	{
		...
		if (!$activated)
			return $this->getTranslator()->trans('This email is already registered and/or invalid.', array(), 'Modules.StNewsletter.Admin');
		...
		return $this->getTranslator()->trans('Thank you for subscribing to our newsletter.', array(), 'Modules.StNewsletter.Admin');
	}
