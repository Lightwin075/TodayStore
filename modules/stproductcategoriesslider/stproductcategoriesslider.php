...
class StProductCategoriesSlider extends BaseSlider implements WidgetInterface
{
	...
	public function installDB()
	{
		$return = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_product_categories_slider` (
                ...
                `pro_per_xxl` tinyint(2) unsigned NOT NULL DEFAULT 6,  
                `pro_per_xl` tinyint(2) unsigned NOT NULL DEFAULT 5, 
                `pro_per_lg` tinyint(2) unsigned NOT NULL DEFAULT 4, 
                `pro_per_md` tinyint(2) unsigned NOT NULL DEFAULT 3, 
                `pro_per_sm` tinyint(2) unsigned NOT NULL DEFAULT 2, 
	...
	public function hookDisplayHome($params, $func=0, $flag=0)
	{
        ...
        if(Configuration::get($this->_prefix_st.'TABS'))
        {
            ...
            if($random)
            {
                if(!$this->_prepareHook('', $display_on))
                    return false;
                $this->smarty->assign(array(
                    'column_slider'         => false,
            ...
            {
                if (!$this->isCached('stproductcategoriesslider_tab.tpl', $this->stGetCacheId($display_on.'-tab')))
            	{
        		...
        		return $this->display(__FILE__, 'stproductcategoriesslider_tab.tpl', $this->stGetCacheId($display_on.'-tab'));
            }
            return false;
        }
        else
    ...
    {        
        ...
        {
            $result = StProductCategoriesSliderClass::getListContent(1,$display_on);
            if(is_array($result) && count($result)) {
                ...
                foreach($result as &$v)
                {
                    if ($v['type'] == 1) {
                        ...
                        $products = Manufacturer::getProducts((int)$v['id_manufacturer'], (int)$this->context->language->id);
                        $manufacturer = new Manufacturer((int)$v['id_manufacturer'], (int)$this->context->language->id, 1, (int)$v['nbr'.$ext], $order_by, $order_way);
                        $v['link'] = $manufacturer->getLink();
    ...
    public function hookDisplayFooter($params, $func='')
    {
	    ...
	    if (!$this->isCached('stproductcategoriesslider-footer.tpl', $this->getCacheId($display_on)))
	    {
		...
		return $this->display(__FILE__, 'stproductcategoriesslider-footer.tpl', $this->getCacheId($display_on));
    }
