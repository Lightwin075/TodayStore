...
$slide = RevGlobalObject::getVar('slide');
$arrSlides = RevGlobalObject::getVar('arrSlides');
$slider_type = RevGlobalObject::getVar('slider_type');
$slideID = RevGlobalObject::getVar('slideID');
        ...
        <ul class="list_slide_links">
            ...
            <li class="<?php echo $staticclass; ?> eg-drag-disabled">
                ...
                if ($slide->isStaticSlide()) {
                    ...
                    RevGlobalObject::setVar('all_slides', $all_slides);
                    ?>
                    <span style="position:absolute; top:13px;left:0px; text-align: center">
                        <span class="setting_text_3"><?php _e("Show Layers from Slide:", 'revslider');
                    ?></span>
                        <select name="rev_show_the_slides">
                            ...
                            foreach ($all_slides as $c_slide) {
                                ...
                                ?>
                                <option value="<?php echo $c_slide->getID();
                                ?>"><?php echo Tools::stripslashes(RevSliderFunctions::getVal($c_params, 'title', 'Slide')) . ' (ID: ' . $c_slide->getID() . ')';
                                ?></option>
                                <?php
                            }
                            ?>
            ...
            foreach ($arrSlides as $t_slide) {
                ...
                $c_bgFitX = (int) ($t_slide->getParam('bg_fit_x', '100'));
                $c_bgFitY = (int) ($t_slide->getParam('bg_fit_y', '100'));

                $c_bgPosition = $t_slide->getParam('bg_position', 'center center');
                $c_bgPositionX = (int) ($t_slide->getParam('bg_position_x', '0'));
                $c_bgPositionY = (int) ($t_slide->getParam('bg_position_y', '0'));
                ...
                if ($c_bgType == 'image' || $c_bgType == 'streamvimeo' || $c_bgType == 'streamyoutube' || $c_bgType == 'streaminstagram') {
                    switch ($slider_type) {
                        case 'posts':
                        case 'specific_posts':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/post.png';
                            break;
                        case 'woocommerce':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/wc.png';
                            break;
                        case 'facebook':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/fb.png';
                            break;
                        case 'twitter':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/tw.png';
                            break;
                        case 'instagram':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/ig.png';
                            break;
                        case 'flickr':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/fr.png';
                            break;
                        case 'youtube':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/yt.png';
                            break;
                        case 'vimeo':
                            $c_urlImageForView = _MODULE_DIR_ . 'revsliderprestashop/views/img/images/sources/vm.png';
                            break;
                    }
                }
                ...
                ?>" class="<?php echo $c_topclass;
                ?>">
                    <a href="<?php echo $urlEditSlide;
                ?>" <?php echo $addParams;
                ?>>
                        <span class="mini-transparent mini-as-bg"></span>
                        <span class="slide-media-container <?php echo $c_bg_extraClass;
                ?>" <?php echo $c_bg_fullstyle;
                ?>></span>
                    ...
                    <div class="slide-link-content">		
                        <span class="slide-link">
                            <span class="slide-link-nr">#<?php echo $slidecounter;
                        ?></span>
                            <input class="slidetitleinput" name="slidetitle" value="<?php echo Tools::stripslashes($title);
                        ?>" />
                        ...
                        <div class="slide-link-toolbar">							
                            ...
                            if ($slidelistID != $slideID && !in_array($slideID, $arrChildrenIDs)) {
                                ?>
                                <a class="slide-link-toolbar-button slide-moveto" href="#"><span class=""><i class="eg-icon-forward"></i><span><?php _e("Copy / Move", 'revslider');
                                ?></span></span></a>
                                <?php
                            }
                            ?>
                            <a class="slide-link-toolbar-button slide-duplicate" href="#"><span class=""><i class="eg-icon-picture"></i><span><?php _e("Duplicate", 'revslider');
                            ?></span></span></a>
                            <a class="slide-link-toolbar-button slide-add-as-template" href="#"><span class=""><i class="eg-icon-menu"></i><span><?php _e("Add to Templates", 'revslider');
                            ?></span></span></a>
                            <a class="slide-link-toolbar-button slide-remove" href="#"><span class=""><i class="eg-icon-trash"></i><span><?php _e("Delete", 'revslider');
                            ?></span></span></a>
                        </div>
                    </div>
