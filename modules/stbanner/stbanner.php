...
class StBanner extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
		...
		$this->description   = $this->getTranslator()->trans('this module was made to easy upload banners in your shop.', array(), 'Modules.StBanner.Admin');
	...
	{ 
        ...
        if ((Tools::isSubmit('bannerdeletestbanner')))
        {
            ...
            if($banner->id)
            {
                ...
                if ($banner->save())
                {
                    //$this->_html .= $this->displayConfirmation($this->getTranslator()->trans('The image was deleted successfully.', array(), 'Admin.Theme.Panda'));  
                    $this->clearBannerCache();
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred while delete banner.', array(), 'Admin.Theme.Panda'));
            }
            else
                $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred while delete banner.', array(), 'Admin.Theme.Panda'));
        }
            ...
            if (!count($error) && $group->validateFields(false))
            {
                ...
                if($group->save())
                {
                    ...
                    if(isset($_POST['savestbannergroupAndStay']) || Tools::getValue('fr') == 'view')
                    {
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Banner group', array(), 'Modules.StBanner.Admin').' '.($id_st_banner_group ? $this->gettranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                }                    
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during banner group', array(), 'Modules.StBanner.Admin').' '.($id_st_banner_group ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
		...
		if (isset($_POST['savestbanner']) || isset($_POST['savestbannerAndStay']))
		{
			...
			if (!count($error) && $banner->validateFields(false) && $banner->validateFieldsLang(false))
            {
                ...
                if($banner->save())
                {
                    ...
                    $this->clearBannerCache();
                    //$this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Banner', array(), 'Modules.StBanner.Admin').' '.($id_st_banner ? $this->gettranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during banner', array(), 'Modules.StBanner.Admin').' '.($id_st_banner ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
                $this->_html .= count($error) ? implode('',$error) : $this->displayError($this->getTranslator()->trans('invalid value for field(s).', array(), 'Admin.Theme.Panda'));
        }
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
					$result['error'][] = $this->getTranslator()->trans('An error occurred during move image.', array(), 'Modules.StBanner.Admin');
				else{
    ...
    public static function getApplyTo()
    {
        ...
        return array(
            ...
            array('name'=>$module->getTranslator()->trans('Categories', array(), 'Admin.Theme.Panda'),'query'=>$category_arr),
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Space between banners:', array(), 'Admin.Theme.Panda'),
                    'name' => 'padding',
                    'default_value' => '',
                    'prefix' => 'px',
                    'class' => 'fixed-width-lg',
                ...
                array(
                    ...
                    'prefix' => 'px',
                    'class' => 'fixed-width-lg',
                    'desc' => $this->getTranslator()->trans('Leave it empty to use the default value 8.', array(), 'Admin.Theme.Panda'),
                ),
                array(
                    'type' => 'text',
        ...
        if($group->id)
        {
            ...
            if ($group->bg_img)
            {
                ...
                $this->fields_form[1]['form']['input']['bg_img_field']['desc'][] = '<div class="image_thumb_block"><img src="'.($image_url).'" width="300" /><div><a class="btn btn-default delete_slider_image" href="javascript:;" data-s_id="'.(int)$group->id.'" data-s_k="bg_img"><i class="icon-trash"></i> '.$this->getTranslator()->trans('Delete', array(), 'Admin.Theme.Panda').'</a></div></div>';
            }
	...
	{        
        ...
        $this->fields_form_banner[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Caption width:', array(), 'Admin.Theme.Panda'),
                    'name' => 'text_width',
                    'options' => array(
                        'query' => array(
                                array('id' => 11, 'name'=>'10%'),
            ...
            $this->fields_form_banner[0]['form']['input'][] = array('type' => 'hidden', 'name' => 'id_st_banner');
             foreach ($languages as $lang)
                if($banner->image_multi_lang[$lang['id_lang']])
                {
                    ...
                    ($lang['id_lang'] != $default_lang ? '<p><a class="btn btn-default st_delete_image" data-id="'.$banner->id.'" data-lang="'.(int)$lang['id_lang'].'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Panda').'</a></p>' : '');
                }
        }
        ...
        $this->fields_form_banner[0]['form']['input'][] = array(
			...
			'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_banner_group='.$group->id.'&viewstbanner&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to list</a><a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.
                Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to main page', array(), 'Admin.Theme.Panda').'</a>',                  
		);
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'&id_st_banner_group='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTRanslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
	protected function initListColumn()
	{
		...
		$this->fields_list_group = array(
			...
			'height' => array(
				'title' => $this->getTranslator()->trans('Height', array(), 'Admin.Theme.Panda'),
				'class' => '',
				'type' => 'text',
    ...
    {
        $navs = array();
        $navs[] = '<a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">'.$this->getTranslator()->trans('Home', array(), 'Admin.Theme.Panda').'</a>';
