...
<div class="block_blog {if isset($classname)} {$classname} {/if}">
    ...
    <div class="pro_second_box pro_block_align_{$stblog.blog_block_align}">
        ...
        <div class="blog_info">
            {if $stblog.display_author && $blog.author}<span class="posted_by mar_r4">{l s='by' d='Shop.Theme.Panda'}</span><span class="link_color">{$blog.author}</span>{/if}
            {if $stblog.display_comment_count && isset($blog.comment_number) && $blog.comment_number}<a href="{$blog.link}#comments" title="{$blog.comment_number} {if $blog.comment_number>1}{l s='Comments' d='Shop.Theme.Panda'}{else}{l s='Comment' d='Shop.Theme.Panda'}{/if}"><i class="fto-chat-1 mar_r4"></i>{$blog.comment_number}</a>{/if}
        ...
        {if $stblog.display_read_more}<a href="{$blog.link}" title="{l s='Read More' d='Shop.Theme.Panda'}" class="{if $stblog.display_read_more==2}go{else}btn btn-default{/if}">{l s='Read More' d='Shop.Theme.Panda'}</a>{/if}
        {/if}
