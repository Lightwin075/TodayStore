...
*}
        ...
        {if $blog.type==1}
            ...
            <div class="blog_image"><a href="{$blog.link}" rel="bookmark" title="{$blog.name}"><img {if $is_lazy}data-src{else}src{/if}="{$blog.cover.links.{$blog_image_type}.image}" alt="{$blog.name}" width="{$blog.cover.links.{$blog_image_type}.width}" height="{$blog.cover.links.{$blog_image_type}.height}" class="{if $is_lazy} swiper-lazy {/if} front-image" /></a></div>
        {/if}
        {/if}
        
        {if $blog.type==2}
        {if isset($blog.galleries) && count($blog.galleries)}
            <div class="swiper-container tm_gallery swiper-button-lr swiper-navigation-circle swiper-small-button" data-lazyload="{$is_lazy}" {if $sttheme.is_rtl} dir="rtl" {/if}>
