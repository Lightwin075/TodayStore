...
class StSwiper extends Module implements WidgetInterface
{
    ...
    private function initPages()
    {
        $this->_pages = array(
                array(
                    'id' => 'index',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('Index', array(), 'Admin.Theme.Panda')
                ),
                array(
        			'id' => 'category',
        			'val' => '2',
        			'name' => $this->getTranslator()->trans('Category', array(), 'Admin.Theme.Panda')
        		),
        		array(
        			'id' => 'product',
        			'val' => '4',
        			'name' => $this->getTranslator()->trans('Product', array(), 'Admin.Theme.Panda')
        		),
                array(
        			'id' => 'pricesdrop',
        			'val' => '8',
        			'name' => $this->getTranslator()->trans('Prices Drop', array(), 'Admin.Theme.Panda')
        		),
                array(
                    'id' => 'newproducts',
                    'val' => '16',
                    'name' => $this->getTranslator()->trans('New Products', array(), 'Admin.Theme.Panda')
                ),
        		array(
        			'id' => 'manufacturer',
        			'val' => '32',
        			'name' => $this->getTranslator()->trans('Manufacturer', array(), 'Admin.Theme.Panda')
        		),
                array(
        			'id' => 'supplier',
        			'val' => '64',
        			'name' => $this->getTranslator()->trans('Supplier', array(), 'Admin.Theme.Panda')
        		),
                array(
        			'id' => 'bestsales',
        			'val' => '128',
        			'name' => $this->getTranslator()->trans('Best Sales', array(), 'Admin.Theme.Panda')
        		),
                array(
        			'id' => 'cms',
        			'val' => '256',
        			'name' => $this->getTranslator()->trans('Cms', array(), 'Admin.Theme.Panda')
        		),
            );
    }
                ...
                'child' => array(
                    array(
                        ...
                        'description' => '<div class="adveditor_a_warper">
                            ...
                            <p class="style_header style_header_1 adveditor_curr width_61" style="color: #ffffff; font-size: 14px;">Panda theme is a modern, clean and professional Prestashop theme, it comes with a lot of useful features. Panda theme is fully responsive, it looks stunning on all types of screens and devices.</p>
	...
	public function getContent()
	{
	    ...
	    if ((Tools::isSubmit('groupstatusstswiper')))
        {
            ...
            if($slide_group->id && $slide_group->toggleStatus())
            {
                ...
                //$this->_html .= $this->displayConfirmation($this->getTranslator()->trans('the status has been updated successfully.', array(), 'Admin.Theme.Panda'));
            }
            else
        ...
        if (Tools::isSubmit('copystswiper'))
        {
            ...
            if($slide->id)
            {
                ...
                $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred when copying.', array(), 'Admin.Theme.Panda'));
        }
			...
			if (!count($error) && $slide_group->validateFields(false) && $slide_group->validateFieldsLang(false))
            {
                ...
                if($slide_group->save())
                {
                    ...
                    if(isset($_POST['savestswipergroupAndStay']) || Tools::getValue('fr') == 'view')
                    {
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Slideshow', array(), 'Admin.Theme.Panda').' '.($id_st_swiper_group ? $this->gettranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                }                    
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred during slideshow', array(), 'Admin.Theme.Panda').' '.($id_st_swiper_group ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
		...
		if (isset($_POST['savestswiper']) || isset($_POST['savestswiperAndStay']))
		{
            ...
            if (!Tools::isSubmit('has_image_'.$default_lang) && (!isset($_FILES['image_multi_lang_'.$default_lang]) || empty($_FILES['image_multi_lang_'.$default_lang]['tmp_name'])))
			{
			    ...
			    $error[] = $this->displayError($this->getTranslator()->trans('image is required at least in ', array(), 'Admin.Theme.Panda').$defaultLanguage->name);
			}
            else
            {
			    $slide->copyFromPost();
			...
			if (!count($error) && $slide->validateFields(false) && $slide->validateFieldsLang(false))
            {
                ...
                if($slide->save())
                {
                    ...
                    $this->clearSwiperCache();
                    //$this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Slide', array(), 'Admin.Theme.Panda').' '.($id_st_swiper ? $this->gettranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred during slide', array(), 'Admin.Theme.Panda').' '.($id_st_swiper ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
                $this->_html .= count($error) ? implode('',$error) : $this->displayError($this->getTranslator()->trans('invalid value for field(s).', array(), 'Admin.Theme.Panda'));
        }
    ...
    public static function getApplyTo()
    {
        ...
        return array(
            array('name'=>$module->getTranSlator()->trans('Hook', array(), 'Admin.Theme.Panda'),'query'=>$location),
            array('name'=>$module->getTranSlator()->trans('Category', array(), 'Admin.Theme.Panda'),'query'=>$category_arr),
            array('name'=>$module->getTranSlator()->trans('CMS', array(), 'Admin.Theme.Panda'),'query'=>$cms_arr),
            array('name'=>$module->getTranSlator()->trans('Manufacturers', array(), 'Admin.Theme.Panda'),'query'=>$manufacturer_arr),
        );
    }
	...
	{        
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                array(
					'type' => 'text',
					'label' => $this->getTranslator()->trans('Group name:', array(), 'Admin.Theme.Panda'),
					'name' => 'name',
                    'size' => 64,
                    'required'  => true,
				),
                array(
                    ...
                    'desc' => '<div class="alert alert-info"><a href="javascript:;" onclick="$(\'#des_page_layout\').toggle();return false;">'.$this->getTranslator()->trans('click here to see hook position', array(), 'Admin.Theme.Panda').'</a>'.
                ...
                'templates' => array(
                    'type' => 'html',
                    'id' => 'style',
                    'label' => $this->gettranslator()->trans('templates:', array(), 'Admin.Theme.Panda'),
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('Direction:', array(), 'Admin.Theme.Panda'),
                    'name' => 'direction',
                    'default_value' => 0,
                    'values' => array(
                        array(
                        ...
                        array(
                            'id' => 'direction_1',
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Vertical, you must set a height to the slider, otherwise it would not work fine', array(), 'Admin.Theme.Panda')),
                    ),
        ...
        $this->fields_form[2]['form'] = array(
            ...
            'input' => array(
                array(
                    ...
                    'class' => 'fixed-width-lg',
                    'desc' => array(
                            $this->getTranslator()->trans('Leave it empty to use default.', array(), 'Admin.Theme.Panda'),                            
                        ),
                ),
                array(
                    'type' => 'select',
        ...
        $this->fields_form[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Spacing between:', array(), 'Admin.Theme.Panda'),
        ...
        $this->fields_form[3]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'desc' => $this->getTranslator()->trans('automatically play animation.', array(), 'Admin.Theme.Panda'),
                    'validation' => 'isUnsignedInt',
                ), 
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Auto height:', array(), 'Admin.Theme.Panda'),
                    'name' => 'auto_height',
                    'default_value' => 0,
                    'is_bool' => true,
                    'values' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Transition style:', array(), 'Admin.Theme.Panda'),
                    ...
                    'desc' => array(
                            $this->getTranslator()->trans('Does not work for template 3.', array(), 'Admin.Theme.Panda'),
                            $this->getTranslator()->trans('Also do not work when this slider has a manually set height, refer to the description of "Height" setting.', array(), 'Admin.Theme.Panda'),
                        ),
                    'validation' => 'isUnsignedInt',
                ),
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Lazy loading:', array(), 'Admin.Theme.Panda'),
                    'name' => 'lazy_load',
                    'default_value' => 1,
                    'is_bool' => true,
                    'values' => array(
                ...
                array(
					'type' => 'color',
					'label' => $this->getTranslator()->trans('prev/next buttons background color:', array(), 'Admin.Theme.Panda'),
					'name' => 'prev_next_bg',
					'class' => 'color',
					'size' => 20,
                    'validation' => 'isColor',
				...
				array(
					'type' => 'color',
					'label' => $this->getTranslator()->trans('prev/next buttons background hover color:', array(), 'Admin.Theme.Panda'),
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('Progress bar:', array(), 'Admin.Theme.Panda'),
                    'name' => 'progress_bar',
                    'default_value' => 0,
                    'values' => array(
                        array(
                        ...
                        array(
                            'id' => 'top',
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Top', array(), 'Admin.Theme.Panda')),
                        array(
                            'id' => 'bottom',
                            'value' => 2,
                            'label' => $this->getTranslator()->trans('Bottom', array(), 'Admin.Theme.Panda')),
                    ),
                    'validation' => 'isUnsignedInt',
                    'desc' => array(
                        $this->getTranslator()->trans('A slider would play automatically, if it has progress bar.', array(), 'Admin.Theme.Panda'),
                        $this->getTranslator()->trans('Tempalte 3 does not have this feature.', array(), 'Admin.Theme.Panda'),
                        ),
                ),
                array(
					'type' => 'color',
					'label' => $this->getTranslator()->trans('Progress bar color:', array(), 'Admin.Theme.Panda'),
					'name' => 'prog_bar_color',
					'class' => 'color',
					'size' => 20,
                    'validation' => 'isColor',
				...
				array(
					'type' => 'color',
					'label' => $this->getTranslator()->trans('progress bar background color:', array(), 'Admin.Theme.Panda'),
					'name' => 'prog_bar_bg',
					'class' => 'color',
					'size' => 20,
                    'validation' => 'isColor',
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Grab Cursor:', array(), 'Admin.Theme.Panda'),
                    'name' => 'mouse_drag',
                    'default_value' => 0,
                    'is_bool' => true,
                    'values' => array(
                    ...
                    'desc' => $this->getTranslator()->trans('User will see the "grab" cursor when hover on Swiper', array(), 'Admin.Theme.Panda'),
                ), 
                array(
                    'type' => 'text',
                ...
                array(
                    ...
                    Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
				),
			),
            'buttons' => array(
                array(
    ...
    public function BuildDropListGroup($slide_group)
    {
        $group = array(
            array(
                'id' => 'items_huge',
                'label' => $this->getTranslator()->trans('Extremely large devices for full screen blocks', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Desktops (>1900px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_xxlg',
                'label' => $this->getTranslator()->trans('Extremely large devices for full screen blocks', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Desktops (>1600px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_xlg',
                'label' => $this->getTranslator()->trans('Extra large devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Desktops (>1400px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_lg',
                'label' => $this->getTranslator()->trans('Large devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Desktops (>1200px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_md',
                'label' => $this->getTranslator()->trans('Medium devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Desktops (>992px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_sm',
                'label' => $this->getTranslator()->trans('Small devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Tablets (>768px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_xs',
                'label' => $this->getTranslator()->trans('Extra small devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Phones (>480px)', array(), 'Admin.Theme.Panda'),
            ),
            array(
                'id' => 'items_xxs',
                'label' => $this->getTranslator()->trans('extremely small devices', array(), 'Admin.Theme.Panda'),
                'tooltip' => $this->getTranslator()->trans('Phones (<480px)', array(), 'Admin.Theme.Panda'),
            ),
        );
	...
	protected function initFormSlide($isbanner=0)
	{
		...
		$this->fields_form_slide[0]['form'] = array(
			...
			'input' => array(
                array(
					'type' => 'select',
        			'label' => $this->getTranslator()->trans('Slideshow:', array(), 'Admin.Theme.Panda'),
		...
		$this->fields_form_slide[1]['form'] = array(
			'legend' => array(
				'title' => $this->getTranslator()->trans('More options', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
			),
			'input' => array(
                array(
                    'type' => 'textarea',
                    'label' => $this->getTranslator()->trans('Caption:', array(), 'Admin.Theme.Panda'),
                    'lang' => true,
                    'name' => 'description',
                    'cols' => 40,
                    'rows' => 10,
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Hide caption on small screen devices:', array(), 'Admin.Theme.Panda'),
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('Alignment:', array(), 'Admin.Theme.Panda'),
                    'name' => 'text_align',
                    'default_value' => 2,
                    'values' => array(
                        array(
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('Caption with:', array(), 'Admin.Theme.Panda'),
                    'name' => 'content_width',
                    'default_value' => 0,
                    'values' => array(
                        array(
                        ...
                        array(
                            ...
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Page width', array(), 'Admin.Theme.Panda')),
                    ),
                    'desc' => $this->gettranslator()->trans('This option would be useful for full screen blocks.', array(), 'Modules.StSwiper.Admin'),
                ), 
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Caption content width:', array(), 'Admin.Theme.Panda'),
                    'name' => 'text_width',
                    'options' => array(
                ...
                array(
                    'type' => 'color',
                    'label' => $this->getTranslator()->trans('Caption color:', array(), 'Admin.Theme.Panda'),
                    'name' => 'text_color',
                    'size' => 33,
                ),
                array(
                    'type' => 'color',
                    'label' => $this->getTranslator()->trans('Caption background:', array(), 'Admin.Theme.Panda'),
                    'name' => 'text_bg',
                    'size' => 33,
                ...
                array(
					...
					'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_swiper_group='.($slide->id_st_swiper_group?$slide->id_st_swiper_group:$id_st_swiper_group).
                        '&viewstswiper&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
				), 
			),
            'buttons' => array(
                array(
		...
		foreach ($languages as $lang)
        {
            $this->fields_form_slide[0]['form']['input']['image_multi_lang_'.$lang['id_lang']] = array(
                    'type' => 'file',
					'label' => $this->getTranslator()->trans('Image', array(), 'Admin.Theme.Panda').' - '.$lang['name'].($default_lang == $lang['id_lang'] ? '('.$this->gettranslator()->trans('default language', array(), 'Admin.Theme.Panda').')' : '').':',
        ...
        if(Validate::isLoadedObject($slide))
        {
            ...
            foreach ($languages as $lang)
                if($slide->image_multi_lang[$lang['id_lang']])
                {
                    ...
                    ($lang['id_lang'] != $default_lang ? '<p><a class="btn btn-default st_delete_image" data-id="'.$slide->id.'" data-lang="'.(int)$lang['id_lang'].'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Panda').'</a></p>' : '');
                }
        }
    ...
    public static function showShortCode($value,$row)
    {
        return '<label data-html="true" data-toggle="tooltip" class="label-tooltip" data-original-title="'.
            Context::getContext()->getTranSlator()->trans('You can copy the short code to anywhere to show the slider if need.', array(), 'Admin.Theme.Panda').
    ...
    public static function showApplyTo($value,$row)
    {
        ...
        elseif($row['id_category'])
        {
            ...
            {
                $module = new StSwiper();
                $result = $category->name.'('.$module->getTranSlator()->trans('Category', array(), 'Admin.Theme.Panda').')';
            }
        }
        ...
        elseif($row['id_cms'])
        {
            ...
            {
                $module = new StSwiper();
                $result = $cms->meta_title.'('.$module->getTranSlator()->trans('CMS', array(), 'Admin.Theme.Panda').')';
            }
        }
        else
        {
            $module = new StSwiper();
            $result = $module->getTranSlator()->trans('--', array(), 'Admin.Theme.Panda');
        }
        return $result;
    }
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'group&id_st_swiper_group='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTRanslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
	...
	protected function initListSlide()
	{
		$this->fields_list = array(
			...
			'id_st_swiper_group' => array(
				'title' => $this->getTranslator()->trans('Group name', array(), 'Admin.Theme.Panda'),
				'width' => 120,
				'type' => 'text',
            ...
            'image_multi_lang' => array(
				'title' => $this->getTranslator()->trans('Image', array(), 'Admin.Theme.Panda'),
				'type' => 'text',
		...
		$helper->toolbar_btn['back'] =  array(
			...
			'desc' => $this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda')
		);

		$helper->title = $this->getTranslator()->trans('Slides', array(), 'Admin.Theme.Panda');
		$helper->table = $this->name;
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'&id_st_swiper='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTRanslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
	protected function initListBanner()
	{
		$this->fields_list_banner = array(
			...
			'id_st_swiper_group' => array(
				'title' => $this->getTranslator()->trans('Group', array(), 'Admin.Theme.Panda'),
				'width' => 120,
				'type' => 'text',
				'callback' => 'showSlideGroupName',
				'callback_object' => 'StSwiper',
		...
		$helper->title = $this->getTranslator()->trans('Banners', array(), 'Admin.Theme.Panda');
		$helper->table = $this->name;
		$helper->orderBy = 'position';
		$helper->orderWay = 'ASC';
	    $helper->position_identifier = 'id_st_swiper';
