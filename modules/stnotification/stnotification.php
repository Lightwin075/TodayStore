...
class StNotification extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
        ...
        self::$location = array(
            array('id' => 0, 'name'=>$this->getTranslator()->trans('Bottom center', array(), 'Admin.Theme.Panda')),
            array('id' => 1, 'name'=>$this->getTranslator()->trans('Bottom right', array(), 'Admin.Theme.Panda')),
            array('id' => 2, 'name'=>$this->getTranslator()->trans('Bottom left', array(), 'Admin.Theme.Panda')),
            array('id' => 3, 'name'=>$this->getTranslator()->trans('Center center', array(), 'Admin.Theme.Panda')),
            array('id' => 4, 'name'=>$this->getTranslator()->trans('Center right', array(), 'Admin.Theme.Panda')),
            array('id' => 5, 'name'=>$this->getTranslator()->trans('Center left', array(), 'Admin.Theme.Panda')),
            array('id' => 6, 'name'=>$this->getTranslator()->trans('Top center', array(), 'Admin.Theme.Panda')),
            array('id' => 7, 'name'=>$this->getTranslator()->trans('Top right', array(), 'Admin.Theme.Panda')),
            array('id' => 8, 'name'=>$this->getTranslator()->trans('Top left', array(), 'Admin.Theme.Panda')),
            array('id' => 9, 'name'=>$this->getTranslator()->trans('Top full screen static', array(), 'Admin.Theme.Panda')),
        );
	}
    ...
    public function sampleData($id_shop)
    {
        ...
        $samples = array(
            array(
                ...
                'delay'         => 2,
                'hide_on_mobile'=> 0,
                'content'       => '<div class="fs_md">We use cookies. If you continue to use this site we will assume that you are happy with it. <a href="#" class="go mar_l4" rel="nofollow" title="Read more">Read more</a></div>',
                'tb_padding'    => 4,
                'lr_padding'    => 6,
                'text_color'    => '#999999',
                'link_hover_color' => '#E54D26',
                'text_bg'       => '#333333',
                'button_color'  => '#ffffff',
        ...
        foreach($samples as $k=>$sample)
        {
            ...
            $module->text_bg        = $sample['text_bg'];
            $module->button_color   = $sample['button_color'];
            $module->button_hover_color = $sample['button_hover_color'];
            $module->button_hover_bg = $sample['button_hover_bg'];
            ...
            if (!count($error) && $notification->validateFields(false) && $notification->validateFieldsLang(false))
            {
                ...
                if($notification->save())
                {
                    ...
                    if(isset($_POST['savestnotificationAndStay']))
                    {
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Settings', array(), 'Admin.Theme.Panda').' '.($id_st_notification ? $this->getTranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                }                    
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during settings', array(), 'Admin.Theme.Panda').' '.($id_st_notification ? $this->getTranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
