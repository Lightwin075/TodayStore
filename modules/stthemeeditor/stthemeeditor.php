...
{	
	...
	public function __construct()
	{
        ...
        $this->tabs = array(
                ...
                array('id'  => '3,27,29,28', 'name' => $this->getTranslator()->trans('Font', array(), 'Admin.Theme.Panda')),
                ...
                array('id'  => '64', 'name' => $this->getTranslator()->trans('CMS page', array(), 'Admin.Theme.Panda')),
        ...
        $this->defaults = array(
            ...
            */
            ...
            'use_view_more_instead'                   => array('exp'=>1,'val'=>0,'smarty_val'=>1),
    ...
    public function initDropListGroup()
    {
        ...
        $this->fields_form[0]['form']['input']['left_column_size']['name'] = $this->BuildDropListGroup($this->findCateProPer(14),0,12);
        $this->fields_form[0]['form']['input']['right_column_size']['name'] = $this->BuildDropListGroup($this->findCateProPer(15),0,12);
    ...
    public function writeCss()
    {
        ...
        */
        ...
        if(Configuration::get('STSN_TOPBAR_B_BORDER_COLOR'))
            $css .='#st_header #top_bar{border-bottom-color:'.Configuration::get('STSN_TOPBAR_B_BORDER_COLOR').';}'; 
        ...
        foreach($toolbar_btn AS $k => $btn)
        {
            if ($btn['type'] == 'dropdown') {
                $html .= '<li class="nav-item dropdown">
                	<a href="javascript:void(0)" class="dropdown-toggle btn btn-default" data-toggle="dropdown">'.$btn['desc'].'<i class="icon-caret-down"></i></a>
                	<ul class="dropdown-menu">
                	<li>
                		<a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&predefineddemostore'.$this->name.'='.$k.'&token='.$token.'" 
                        onclick="if (confirm(\''.$this->getTranslator()->trans('Importing %s%, are your sure?', array('%s%'=>$btn['desc']), 'Admin.Theme.Panda').'\')){return true;}else{event.preventDefault();}"
                        title="'.$this->getTranslator()->trans('All data from ', array(), 'Modules.StThemeeditor.Admin').$btn['desc'].'">
                    ...
                    <li>
                        ...
                        onclick="if (confirm(\''.$this->getTranslator()->trans('Importing colors only from %s%, are your sure?', array('%s%'=>$btn['desc']), 'Admin.Theme.Panda').'\')){return true;}else{event.preventDefault();}"
                        title="'.$this->getTranslator()->trans('Colors are set on the Theme editor module from ', array(), 'Modules.StThemeeditor.Admin').$btn['desc'].'">
                ...
                $html .= '
                <li class="nav-item"><a id="desc-configuration-'.$k.'" class="boolbtn-'.$k.' btn btn-default" onclick="return import_demo(this)" href="'.$btn['href'].'" title="'.$btn['desc'].'">
                <span>
                <i class="'.$btn['class'].'"></i> '.$btn['desc'].'</span></a></li>';    
            }
        }
        $html .= '</ul>';
