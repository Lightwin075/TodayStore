...
{block name="input"}
	{if $input.type == 'row_generater'}
	    <div class="col-lg-12">
	    	<p>{l s='How many columns do you want to have for this row' d='Admin.Theme.Panda'}</p>
			...
			<p>{l s='Set with for each column' d='Admin.Theme.Panda'}</p>
			...
			<p class="help-block">{l s='Column names can be empty' d='Admin.Theme.Panda'}</p>
			<p class="help-block">{l s='If the sum of all column widths is larger than 1 in a row, then extra columns would not be displayed on the front office. For example, you have 4/12, 3/12, 4/12 and 5/12, then the last 5/12 would not be displayed.' d='Admin.Theme.Panda'}</p>
		</div>
		...
		<div class="fontello_wrap">
			<a id="btn_{$input.name}" class="btn btn-default" data-toggle="modal" href="#" data-target="#modal_{$input.name}">
				<i class="{$fields_value[$input.name]}"></i>{l s='Edit' d='Admin.Theme.Panda'}
			</a>
			<div class="modal fade" id="modal_{$input.name}" tabindex="-1">
				<div class="modal-dialog">
					...
					<div class="modal-header">
						...
						<h4 class="modal-title">{l s="Icon" d='Admin.Theme.Panda'}</h4>
					</div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">{l s="OK" d='Admin.Theme.Panda'}</button>
					</div>
					</div>
				</div>
			</div>
	...
	{elseif $input.type == 'fontello_list'}
		<div class="fontello_wrap">
			<a id="btn_{$input.name}" class="btn btn-default" data-toggle="modal" href="#" data-target="#modal_fontello">
				{l s='Click here' d='Admin.Theme.Panda'}
			</a>
			<div class="modal fade" id="modal_fontello" tabindex="-1">
				<div class="modal-dialog">
					...
					<div class="modal-body">
						...
						<p>{l s='You can use this code to add a icon to your custom content, put it in cource code.' d='Admin.Theme.Panda'} <a href="javascript:;" onclick="{literal}$('#how_to_put_icons').toggle();return false;{/literal}">{l s='Here is a screenshot' d='Admin.Theme.Panda'}</a>. {l s='Alert fto-diamond as you needed. "fto_1x" is used to change the size of icons, here are some other classes:' d='Admin.Theme.Panda'} fto_small,fto_large,fto_0x,fto_1x,fto_2x...</p>
						<p id="how_to_put_icons" style="display:none;">
							<img src="{$smarty.const._MODULE_DIR_}st_easycontent/views/img/how_to_put_icons.jpg" />
						</p>
			...
			<style type="text/css">
				@font-face {
				  font-family: 'fontello';
				  src: url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.eot');
				  src: url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.eot#iefix') format('embedded-opentype'),
				       url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.woff2') format('woff2'),
				       url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.woff') format('woff'),
				       url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.ttf') format('truetype'),
				       url('{$input.values.module_name}../../themes/{$input.values.theme_name}/_dev/font/fontello.svg#fontello') format('svg');
				  font-weight: normal;
				  font-style: normal;
				}
				{$input.values.css}
		...
		<div class="pre_box">
			...
			{foreach $input.values as $k_t => $v_t}
				<li class="{if $input_value[0] == $k_t} easy_selected {/if}" id="easy_pre_temp_{$k_t}" data-pre_template="{$k_t}">
					...
					<div class="easy_image_name">{l s='Template' d='Admin.Theme.Panda'} {$k_t}</div>
				</li>
			{/foreach}
			</ul>
		</div>
			...
			<div class="row">
				{foreach $input.values.medias as $media=>$mv}
					<div class="col-xs-4 col-sm-3">
						<label data-html="true" data-toggle="tooltip" class="label-tooltip" data-original-title="{if $media=='fw'}{l s='If this is set and this module is hooked to the displayFullWidthXXX hooks, then this module will be displayed in full screen.' d='Admin.Theme.Panda'}{elseif $media=='xxl'}{l s='Desktops (>1400px)' d='Admin.Theme.Panda'}{elseif $media=='xl'}{l s='Desktops (>1200px)' d='Admin.Theme.Panda'}{elseif $media=='lg'}{l s='Desktops (>992px)' d='Admin.Theme.Panda'}{elseif $media=='md'}{l s='Tablets (>768px)' d='Admin.Theme.Panda'}{elseif $media=='sm'}{l s='Phones (>544px)' d='Admin.Theme.Panda'}{elseif $media=='xs'}{l s='Phones (<544px)' d='Admin.Theme.Panda'}{/if}">{if $media=='fw'}{l s='Full screen' d='Admin.Theme.Panda'}{elseif $media=='xxl'}{l s='Extra large devices' d='Admin.Theme.Panda'}{elseif $media=='xl'}{l s='Large devices' d='Admin.Theme.Panda'}{elseif $media=='lg'}{l s='Medium devices' d='Admin.Theme.Panda'}{elseif $media=='md'}{l s='Small devices' d='Admin.Theme.Panda'}{elseif $media=='sm'}{l s='Extra small devices' d='Admin.Theme.Panda'}{elseif $media=='xs'}{l s='Extremely small devices' d='Admin.Theme.Panda'}{/if}</label>
		...
		<!-- <button id="toggle_tinymce" type="button" class="btn btn-primary">{l s='Toggle tinymce text editor' d='Admin.Theme.Panda'}</button> -->
			...
			<div class="modal fade" id="modal_adv_editor" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
				<div class="modal-dialog modal-lg" role="document">
					...
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
						<h4 class="modal-title">{l s='Text editor' d='Admin.Theme.Panda'} <a href="{$input.name_blank}" target="_blank" title="{l s='Open in new window' d='Admin.Theme.Panda'}">{l s='Open in new window' d='Admin.Theme.Panda'}</a></h4>
					</div>
					<div class="modal-body">
