...
class StEasyContent extends Module implements WidgetInterface
{
	...
	function __construct()
	{
        ...
        self::$type = array(
            ...
            20 => array('value' => 20, 'id' => 'type_20', 'label' => $this->getTranslator()->trans('Blog article page', array(), 'Modules.StEasycontent.Admin')),
        );
    ...
    private function initElements()
    {
        $this->_elements = array(
                ...
                2 => array(
                    'id' => 2,
                    'name' => $this->getTranslator()->trans('Plan text, Text with image', array(), 'Modules.StEasycontent.Admin'),
                ),
                8 => array(
                    'id' => 8,
	...
	protected function initForm()
	{
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
					'type' => 'select',
        			'label' => $this->getTranslator()->trans('Specify category or manufacturer:', array(), 'Modules.StEasycontent.Admin'),
                    'onchange' => 'handle_apply_to(this,\''.
                        $this->getTranslator()->trans('Set ID product', array(), 'Modules.StEasycontent.Admin').'\',\''.
                        $this->getTranslator()->trans('Product ID', array(), 'Modules.StEasycontent.Admin').'\',\''.
                        $this->getTranslator()->trans('Choose ID product', array(), 'Modules.StEasycontent.Admin').'\');',
                ...
                array(
                    ...
                    'desc' => $this->getTranslator()->trans('This option works for custom content blocks on FullWidth* hooks, cms pages, blog pages.', array(), 'Modules.StEasycontent.Admin'),
                ),
                array(
					'type' => 'text',
        ...
        $this->fields_form[2]['form'] = array(
            ...
            'input' => array(
                ...
                'bg_img_field' => array(
                    ...
                    'name' => 'bg_img',
                    'desc' => array(
                            $this->getTranslator()->trans('Generally a background image should be higher that the height of this block.', array(), 'Modules.StEasycontent.Admin'),
                        ),
                ),
                array(
                    'type' => 'text',
    ...
    public function createLinks($check_id_product=0)
    {
        ...
        if($check_id_product)
            $product_arr[] = array('id'=>'1_'.$check_id_product,'name'=>$this->getTranslator()->trans('Product ID', array(), 'Modules.StEasycontent.Admin').' '.$check_id_product);
        else
            $product_arr[] = array('id'=>'PRODUCT','name'=>$this->getTranslator()->trans('Choose ID product', array(), 'Modules.StEasycontent.Admin'));
    ...
    public static function displayLocation($value, $tr)
    {
        ...
        $result = $context->getTranslator()->trans('--', array(), 'Admin.Theme.Panda');
        switch((int)$tr['type']) {
            case 1:
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'element&id_st_easy_content_element='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTranslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$this->_checkImageDir() || !$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
					$result['error'][] = $this->getTranslator()->trans('An error occurred during move image.', array(), 'Modules.StEasycontent.Admin');
				else{
    ...
    public function loadImageFieldsDesc(&$fields_form = array(), $object)
    {
        ...
        foreach($fields_form AS &$field) {
            if (isset($field['type']) && $field['type'] == 'file') {
                if (isset($object->$field['name']) && $image = $object->$field['name']) {
