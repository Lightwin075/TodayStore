...
class StSocial extends Module implements WidgetInterface
{
		...
		if (isset($_POST['savestsocial']) || isset($_POST['savestsocialAndStay']))
		{
			...
			if ($social->validateFields(false) && $social->validateFieldsLang(false))
            {
                ...
                if($social->save())
                {
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred during social button ', array(), 'Modules.StSocial.Admin').' '.($id_st_social ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
                $this->_html .= $this->displayError($this->getTranslator()->trans('invalid value for field(s).', array(), 'Admin.Theme.Panda'));
        }
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Setting', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
            ),
            'input' => array(
                array(
                    ...
                    'desc' => array(
                        ...
                        '<a href="https://www.pinterest.com/pin/create/button/?url=https%3A%2F%2Fpanda.sunnytoo.com%2Fen%2Ffashion%2F8-hot-new-women-s-thicken-warm-winter-coat-hood-parka-overcoat-long-jacket-outwear-1234567890123.html&media=http%3A%2F%2Fw-static2.sunnytoo.net%2F13-thickbox_default%2Fhot-new-women-s-thicken-warm-winter-coat-hood-parka-overcoat-long-jacket-outwear.jpg&description=hot%20new%20women%27s%20thicken%20warm%20winter%20coat%20hood%20parka%20overcoat%20long%20jacket%20outwear" target="_blank">Click here to see how the above link works.</a>',
                    ),
                ),
                array(
                    'type' => 'text',
    ...
    protected function initSettingForm()
    {
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('Size:', array(), 'Admin.Theme.Panda'),
                    'name' => 'links_size',
                    'default_value' => 1,
                    'values' => array(
                        array(
                            'id' => 'links_size_0',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Samll', array(), 'Admin.Theme.Panda')),
                        array(
                            'id' => 'links_size_1',
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Medium', array(), 'Admin.Theme.Panda')),
                        array(
                            'id' => 'links_size_2',
                            'value' => 2,
                            'label' => $this->getTranslator()->trans('Large', array(), 'Admin.Theme.Panda')),
                    ),
                    'validation' => 'isUnsignedInt',
                    'desc' => $this->getTranslator()->trans('Does not work for links in hover fly buttons and sidebar.', array(), 'Modules.StSocial.Admin'),
                ),
        ...
        if ($logo_img = Configuration::get($this->_prefix_st.'LOGO_IMG'))
        {
            ...
            $this->fields_form[0]['form']['input']['logo_img_field']['desc'] .= '<img width=200 src="'.($logo_img).'" /><p><a class="btn btn-default st_delete_image" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans('Delete', array(), 'Admin.Theme.Panda').'</a></p>';
        }
