...
class StStickers extends Module implements WidgetInterface
{
		...
		if (isset($_POST['save'.$this->name]) || isset($_POST['save'.$this->name.'AndStay']))
		{
			...
			if (!count($error) && $sticker->validateFields(false) && $sticker->validateFieldsLang(false))
            {
                ...
                if($sticker->save())
                {
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during sticker', array(), 'Modules.StStickers.Admin').' '.($id_st_sticker ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
	...
	protected function initForm()
	{
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Border width:', array(), 'Admin.Theme.Panda'),
                    'name' => 'border_width',
                    'prefix' => 'px',
                    'class' => 'fixed-width-lg',
                ),
        ...
        if(Validate::isLoadedObject($sticker))
        {
            foreach ($languages as $lang)
                if($sticker->image_multi_lang[$lang['id_lang']])
                {
                    ...
                    ($lang['id_lang'] != $default_lang ? '<p><a class="btn btn-default st_delete_image" data-id="'.$sticker->id.'" data-lang="'.(int)$lang['id_lang'].'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Panda').'</a></p>' : '');
                }
        }
    ...
    protected function initMapForm()
	{
        ...
        $this->fields_form[0]['form']['input']['products']['desc'] = $this->getTranslator()->trans('Type product name to add prodcuts:', array(), 'Modules.StStickers.Admin').'<br/>'.$this->getTranslator()->trans('Current products', array(), 'Admin.Theme.Panda')
