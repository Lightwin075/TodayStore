...
class StProductComments extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
        ...
        $this->description = $this->getTranslator()->trans('Allows users to post comments for products and ask pre-sale questions.', array(), 'Modules.StProductcomments.Admin');
    ...
    public function installDb()
	{
        ...
        $return &= Db::getInstance()->execute('
            ...
            PRIMARY KEY (`id_st_product_comment`, `id_st_product_comment_criterion`),
        ...
        $this->_hooks = array(
            'Hooks' => array(
                ...
                array(
                    ...
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayFooterBefore', array(), 'Admin.Theme.Panda')
                ),
                array(
                    'id' => 'displayHomeLeft',
                    'val' => '1',
            ...
            'Footer' => array(      
                array(
                    ...
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter1', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),   
                array(
                    'id' => 'displayStackedFooter2',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter2', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),   
                array(
                    'id' => 'displayStackedFooter3',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter3', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),   
                array(
                    'id' => 'displayStackedFooter4',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter4', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),   
                array(
                    'id' => 'displayStackedFooter5',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter5', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),   
                array(
                    'id' => 'displayStackedFooter6',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayStackedFooter6', array(), 'Admin.Theme.Panda'),
                    'in_footer'=>1,
                ),
                array(
                    'id' => 'displayFooter',
    ...
    public function getContent()
	{
        ...
        if (Tools::isSubmit('add'.$this->name) || (Tools::isSubmit('update'.$this->name) && $id_st_product_comment))
        {
            ...
            $tabs = array();
            $tabs = array(
                array('id'  => '0,1,5,6', 'name' => $this->getTranslator()->trans('General settings', array(), 'Admin.Theme.Panda')),
                ...
                foreach($form['form']['input'] as $field)
                    if(isset($field['validation']))
                    {
                        ...
                        if (isset($field['required']) && $field['required'] && $value==false && (string)$value != '0')
        						$errors[] = $this->getTranslator()->trans('Field "%s%" is required.', array('%s%' => $field['label']), 'Admin.Theme.Panda');
                        elseif($value)
                        {
                            $field_validation = $field['validation'];
        					if (!Validate::$field_validation($value))
                                $errors[] = $this->getTranslator()->trans('Field "%s%" is invalid.', array('%s%' => $field['label']), 'Admin.Theme.Panda');
                        }
        				// Set default value
    ...
    protected function renderCommentsList()
    {
        ...
        if ($id_st_product_comment) {
            ...
            $helper->toolbar_btn['back'] =  array(
                'href' => AdminController::$currentIndex.'&configure='.$this->name.($parent->id_parent ? '&id_st_product_comment='.$parent->id_parent.'&view'.$this->name : '').'&token='.Tools::getAdminTokenLite('AdminModules').($parent->id_parent ? '' : '#3'),
            ...
            $fields_list['validate'] = array(
                'title' => $this->getTranslator()->trans('Approved', array(), 'Admin.Theme.Panda'),
                'active' => 'status',
                'type' => 'bool',
                );
        } else {
            $helper->toolbar_btn['edit'] =  array(
    			...
    			'desc' => $this->getTranslator()->trans('Settings', array(), 'Admin.Theme.Panda'),
    		);
        }
    ...
    public function getStandardFieldList()
    {
        return array(
            ...
            'content' => array(
                'title' => $this->trans('Content', array(), 'Admin.Theme.Panda'), 
                'type' => 'text',
                'callback' => 'showContent',
                'callback_object' => $this,
                ),
            'name' => array(
                'title' => $this->trans('Product', array(), 'Admin.Theme.Panda'), 
                'type' => 'text',
                ),
            'unapproved' => array(
                'title' => $this->trans('Unapproved replies', array(), 'Admin.Theme.Panda'), 
                'type' => 'text',
                'callback' => 'showUnapproved',
                'callback_object' => $this,
                ),
            'home_featured' => array(
                'title' => $this->getTranslator()->trans('Home featured', array(), 'Admin.Theme.Panda'),
                'active' => 'status',
                'type' => 'bool',
                ),
            'date_add' => array(
    ...
    {
        $html = $value.'<br>'.
            $this->getTranslator()->trans('Rating: ', array(), 'Modules.StProductcomments.Admin').$row['grade'].' / 5, '.
    ...
    public function displayApproveLink($token, $id, $name = null)
    {
        $this->smarty->assign(array(
            ...
            'action' => $this->getTranslator()->trans('Approve', array(), 'Admin.Theme.Panda'),
        ));
    ...
    protected function initConfigFormFields()
    {
		...
		$this->fields_form[0]['form'] = array(
            ...
            'description' => $this->getTranslator()->trans('Save your changes before go to another tab', array(), 'Admin.Theme.Panda'),
        ...
        $this->fields_form[5]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Testimonial Page', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
            ),
        ...
        $this->fields_form[8]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Home slider', array(), 'Admin.Theme.Panda'),
                'icon'  => 'icon-cogs'
            ),
    ...
    public function renderCommentForm()
	{
        ...
        if (!$comment->id) {
            ...
            $parent = new StProductCommentClass($id_parent);
            $comment->is_admin = 1;
        }
		$this->fields_form[0]['form'] = array(
			'legend' => array(
			...
			'input' => array(
				...
				/*array(
					'type' => 'text',
					'label' => $this->trans('Review title:', array(), 'Modules.StProductcomments.Admin'),
					'name' => 'title',
				),*/
                array(
					'type' => 'textarea',
                    'id'   => 'cm_content',
					'label' => $this->trans('Content:', array(), 'Admin.Theme.Panda'),
					'name' => 'content',
                    'required' => true,
					'rows' => 8,
					'cols' => 60,
                 ...
                 array(
    				'type' => 'switch',
    				'label' => $this->trans('Approved:', array(), 'Admin.Theme.Panda'),
    				'name' => 'validate',
    				'required' => false,
    				'is_bool' => true,
    				'values' => array(
    					array(
    						'id' => 'validate_on',
    						'value' => 1,
    						'label' => $this->trans('Yes', array(), 'Admin.Theme.Panda')
    					),
    					array(
    						'id' => 'validate_off',
    						'value' => 0,
    						'label' => $this->trans('No', array(), 'Admin.Theme.Panda')
    					)
    				),
    			),
                'featured' => array(
                ...
                array(
    				...
    				'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.
                        ...
                        '&token='.Tools::getValue('token').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
    			),
    		),
            'buttons' => array(
                array(
    ...
    public function renderCriterionForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Active', array(), 'Admin.Theme.Panda'),
                    'name' => 'active',
                    'values' => array(
                        array(
                            'id' => 'active_on',
    ...
    public function hookDisplayProductExtraContent($params)
    {
        ...
        $extraContent->setTitle(
            $this->getTranslator()->trans('Reviews', array(), 'Shop.Theme.Panda').
            ($nbComments ? '('.$nbComments.')' : '')
        )->setContent(
    ...
    public function hookDisplayHeader($params)
    {
        ...
        Media::addJsDef(array(
            'confirm_report_message' => $this->getTranslator()->trans('Are you sure report abuse ?', array(), 'Admin.Theme.Panda'),
            'dictRemoveFile' => $this->getTranslator()->trans('Remove', array(), 'Admin.Theme.Panda'),
            'st_pc_display_format' => $this->getTranslator()->trans('#left / #max Characters left.', array(), 'Admin.Theme.Panda'),
        ...
        if (!$this->isCached($template, $this->stGetCacheId('header')))
        {
            ...
            if ($text_color = Configuration::get($this->_prefix_st.'TEXT_COLOR'))
                $custom_css .= '.'.$classname.' .pcomments_item,.'.$classname.' .pcomments_item a{color:'.$text_color.';}';
    ...
    public function getConfigError()
    {
        ...
        $errors = array(
            1 => $this->getTranslator()->trans('An error occurred during deleting.', array(), 'Admin.Theme.Panda'),
            1 => $this->getTranslator()->trans('An error occurred during deleting section.', array(), 'Admin.Theme.Panda'),
            3 => $this->getTranslator()->trans('An error occurred during creation.', array(), 'Admin.Theme.Panda'),
            4 => $this->getTranslator()->trans('An error occurred during updating.', array(), 'Admin.Theme.Panda'),
            5 => $this->getTranslator()->trans('An error occurred while updating the status.', array(), 'Admin.Theme.Panda'),
            6 => $this->getTranslator()->trans('The settings updated failed.', array(), 'Admin.Theme.Panda'),
            7 => $this->getTranslator()->trans('Invalid value for fields.', array(), 'Admin.Theme.Panda'),
        );
