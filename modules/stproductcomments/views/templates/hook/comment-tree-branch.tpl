...
<div id="pcomment-{$node.id_st_product_comment}" class="line_item">
    <div class="flex_container flex_column_md flex_start px-2 pcomment-for-reply" {if $pcomments.g_rich_snippets} itemprop="review" itemscope itemtype="https://schema.org/Review"{/if}>
        ...
        <div class="pcomment_right flex_child">
            ...
            <a href="{$image}" class="{if isset($pc_image_size)}pcomments_images_large{else}pcomments_images_small{/if} flex_box mr-2 mb-1 st_popup_image inline_block" title="{l s='Click to zoom out' d='Shop.Theme.Panda'}" rel="nofollow"><img src="{$image}" class="general_border" alt="{l s='Click to zoom out' d='Shop.Theme.Panda'}" /></a>
            {/foreach}
            </div>
            {/if}
            ...
            <div class="flex_container">
                <div class="pcomment_rbl flex_child">
                ...
                <div class="pcomment_rbr">
                    {if !isset($node.customer_report) || !$node.customer_report}
                    <a href="javascript:;" class="report_btn btn-spin js-btn-active mr-2 text_color" data-id-st-product-comment="{$node.id_st_product_comment}" title="{l s='Report abuse' d='Shop.Theme.Panda'}" rel="nofollow">{l s='Report abuse' d='Shop.Theme.Panda'}</a>
                    {/if}
                    {if $pcomments.helpful!=2}
                        <span class="mr-1">{l s='Helpful?' d='Shop.Theme.Panda'}</span>
                        {if isset($node.customer_advice) && $node.customer_advice}
                        <span class="mr-2">
                        {else}
                        <a href="javascript:;" class="usefulness_btn btn-spin js-btn-active mr-2" data-is-usefull="1" data-id-st-product-comment="{$node.id_st_product_comment}" title="{l s='Yes' d='Shop.Theme.Panda'}" rel="nofollow">
                        {/if}
