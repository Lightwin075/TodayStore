...
<div><span class="mr-2 pc_testi_author">{l s='By' d='Shop.Theme.Panda'}: {$node.customer_name}</span><span class="pc_testi_date">{include file='module:stproductcomments/views/templates/hook/timeago.tpl' timeago=$node.timeago date_add=$node.date_add}</span></div>
</div>