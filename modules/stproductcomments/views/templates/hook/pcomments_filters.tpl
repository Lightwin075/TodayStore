...
<div class="flex_container flex_left general_top_border general_bottom_border p-2 mb-2 general_bg">
  <a href="{$pcomments.clear_all_link}" class="{if $pcomments.id_product} pc-search-link {/if} mr-3 {if !$pcomments.does_filter_by_star && !$pcomments.does_filter_by_pic} theme_color {/if}" rel="nofollow" title="{l s='All reviews' d='Shop.Theme.Panda'}">{l s='All reviews' d='Shop.Theme.Panda'}<span class="pcomment_stat">({$pcomments.stat.total})</span></a>
  <div class="dropdown_wrap mr-3">{strip}
        <div class="dropdown_tri dropdown_tri_in" aria-haspopup="true" aria-expanded="false">
          ...
          {/foreach}
          <span class="{if $pcomments.does_filter_by_star} theme_color {/if}">{if $current_star_lable}{$current_star_lable.label}<span class="pcomment_stat">({$current_star_lable.val})</span>{else}{l s='All stars' d='Shop.Theme.Panda'}<span class="pcomment_stat">({$pcomments.stat.total})</span>{/if}</span>
  ...
  <a href="{$pcomments.filter_pic}" class="{if $pcomments.id_product} pc-search-link {/if} mr-3 {if $pcomments.does_filter_by_pic} theme_color {/if}" rel="nofollow" title="{l s='With pictures' d='Shop.Theme.Panda'}" >{l s='With pictures' d='Shop.Theme.Panda'}<span class="pcomment_stat">({$pcomments.stat.image_total})</span></a>
  ...
  <div class="dropdown_wrap flex_float_right">{strip}
        <div class="dropdown_tri dropdown_tri_in" aria-haspopup="true" aria-expanded="false">
          ...
          {/foreach}
          <span>{if $current_sort_order}{$current_sort_order}{else}{l s='Sort order' d='Shop.Theme.Panda'}{/if}</span>
          <i class="fto-angle-down arrow_down arrow"></i>
          <i class="fto-angle-up arrow_up arrow"></i>
        </div>
        {/strip}
