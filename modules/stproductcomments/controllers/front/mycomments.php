...
class StProductCommentsMyCommentsModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        if (Tools::getValue('SaveComment')) {
    		...
    		if (!Validate::isInt($id_product) || !Validate::isInt($id_order_detail) || Tools::getValue('secure_key') != $this->secure_key)
    			$this->errors[] = $this->trans('Invalidate Parameeters', array(), 'Shop.Theme.Panda');
    		if (!Tools::getValue('content') || !Validate::isMessage(Tools::getValue('content')))
    			$this->errors[] = $this->trans('Your comment is too short. Please try again', array(), 'Shop.Theme.Panda');
    		if (!$this->context->customer->id)
    			$this->errors[] = $this->trans('Please login to post a comment', array(), 'Shop.Theme.Panda');
    
    		$product = new Product(Tools::getValue('id_product'));
    		if (!$product->id)
    			$this->errors[] = $this->trans('Product not found', array(), 'Shop.Theme.Panda');
    		...
    		if (!count($this->errors)) {
                ...
                $this->success[] = $this->trans('Thank you for your comment.', array(), 'Shop.Theme.Panda');
                if (Configuration::get($this->_prefix_st.'MODERATE')) {
                    $this->success[] = $this->trans('Your comment may be awaiting moderation before being published.', array(), 'Shop.Theme.Panda');    
                }
            ...
            if ($comment->id && $comment->delete()) {
                $this->success[] = $this->trans('Comment was deleted.', array(), 'Shop.Theme.Panda');
            } else {
                $this->errors[] = $this->trans('Error occurred when deleting comment.', array(), 'Shop.Theme.Panda'); 
            }
	...
	public function initContent()
	{
        ...
        if (($id_order = Tools::getValue('id_order')) && ($id_order_detail = Tools::getValue('id_order_detail'))) {
            if (Tools::getValue('secure_key') != $this->secure_key) {
                echo $this->trans('Invalid secure key', array(), 'Shop.Theme.Panda');
                exit;
            }
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = array(
            'title' => $this->trans('Product comments', array(), 'Shop.Theme.Panda'),
