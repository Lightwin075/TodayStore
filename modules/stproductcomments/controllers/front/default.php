...
class StProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
    ...
    protected function valideRecaptcha($response= '')
    {
        ...
        if (!($secret_key  = Configuration::get($this->_prefix_st.'RECAPTCHA_SECRET_KEY')) || !Configuration::get($this->_prefix_st.'RECAPTCHA_SITE_KEY')) {
            return $this->trans('Recaptcha site key and secret key required.', array(), 'Shop.Theme.Panda');
        }
        if (!$response) {
            return $this->trans('Recaptcha validate failed.', array(), 'Shop.Theme.Panda');
        }
	...
	protected function ajaxProcessAddComment()
	{
		...
		if (!Validate::isInt(Tools::getValue('id_product')))
			$errors[] = $this->trans('Invalidate Product ID ', array(), 'Shop.Theme.Panda');
		if (!Tools::getValue('content') || !Validate::isMessage(Tools::getValue('content')))
			$errors[] = $this->trans('Your comment is too short. Please try again', array(), 'Shop.Theme.Panda');
		if (!$id_customer && (!Tools::isSubmit('customer_name') || !Tools::getValue('customer_name') || !Validate::isGenericName(Tools::getValue('customer_name'))))
			$errors[] = $this->trans('Customer name is empty', array(), 'Shop.Theme.Panda');
		if (!$this->context->customer->id && !Configuration::get($this->_prefix_st.'ALLOW_GUESTS'))
			$errors[] = $this->trans('Please login to post a comment', array(), 'Shop.Theme.Panda');

		$product = new Product(Tools::getValue('id_product'));
		if (!$product->id)
			$errors[] = $this->trans('Product not found', array(), 'Shop.Theme.Panda');
		...
		if (!count($errors))
		{
			...
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + Configuration::get($this->_prefix_st.'MINIMAL_TIME')) < time()))
			{
                ...
                if ($image = Tools::getValue('image')) {
                    Db::getInstance()->insert('st_product_comment_image', 
                        array('id_st_product_comment'=>$comment->id, 'image' => pSQL($image))
                    );
                }
                
                $result['r'] = true;
                $result['thank'] = $this->trans('Thank you for your comment.', array(), 'Shop.Theme.Panda');
                $result['moderation'] = Configuration::get($this->_prefix_st.'MODERATE') ? '<br/>'.$this->trans('Your comment may be awaiting moderation before being published.', array(), 'Shop.Theme.Panda') :  '';
			}
			else
			{
                $result['m'] = $this->trans('You should wait %seconds% seconds before posting a new comment.', array('%seconds%'=>Configuration::get($this->_prefix_st.'MINIMAL_TIME')), 'Shop.Theme.Panda');
			}
		}
		else
    ...
    protected function ajaxProcessAddReply()
	{
		...
		if (!$comment->id)
			$errors[] = $this->trans('Invalidate parameter ', array(), 'Shop.Theme.Panda');
		if (!Tools::getValue('content') || !Validate::isMessage(Tools::getValue('content')))
			$errors[] = $this->trans('Your comment is too short. Please try again', array(), 'Shop.Theme.Panda');
        $customer_name = Tools::getValue('customer_name');
        if (!Validate::isGenericName($customer_name))
            $errors[] = $this->trans('Invalidate name', array(), 'Shop.Theme.Panda');
		...
		if (!count($errors))
		{
            ...
            $result['r'] = true;
            $result['thank'] = $this->trans('Thank you for your reply.', array(), 'Shop.Theme.Panda');
            $result['moderation'] = Configuration::get($this->_prefix_st.'MODERATE') ? '<br/>'.$this->trans('Your comment may be awaiting moderation before being published.', array(), 'Shop.Theme.Panda') :  '';
		}
		else
            $result['m'] = implode('<br/>',$errors);
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
					$result['error'] = $this->trans('An error occurred during move image.', array(), 'Shop.Theme.Panda');
				else{
                   ...
                   if(!ImageManager::resize($temp_name, _PS_UPLOAD_DIR_.'stproductcomments/'.$c_name.'.'.$type, null, null, $type))
				       $result['error'] = $this->trans('An error occurred during the image upload.', array(), 'Shop.Theme.Panda');
				} 
				if (isset($temp_name))
					@unlink($temp_name);
            ...
            $result['error'] = $this->trans('Please upload an image.', array(), 'Shop.Theme.Panda');
        }
        die(Tools::jsonEncode($result));
    }
