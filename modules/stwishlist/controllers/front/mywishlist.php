...
class StWishListMyWishListModuleFrontController extends ModuleFrontController
{
	...
	public function initContent()
	{
		...
		elseif (!empty($action) && method_exists($this, 'ajaxProcess'.Tools::toCamelCase($action)))
			...
			die(Tools::jsonEncode(array('error' => $this->trans('Method doesn\'t exist', array(), 'Shop.Theme.Panda'))));
	}
		...
		if ($this->context->customer->isLogged())
		{
			...
			if (Tools::isSubmit('submitWishlist')) {
				if (Configuration::get('PS_TOKEN_ACTIVATED') == 1 && strcmp(Tools::getToken(), Tools::getValue('token'))) {
				    $errors[] = $this->trans('Please refresh the page and try again', array(), 'Shop.Theme.Panda');
				}
				if (!count($errors)) {
					$name = Tools::getValue('name');
                    $name = trim($name);
					if (empty($name)) {
					   $errors[] = $this->trans('You must specify a name.', array(), 'Shop.Theme.Panda');
					}
					/*if (StWishListClass::isExistsByNameForUser($name)) {
					   $errors[] = $this->trans('This name is already used by another list.', array(), 'Shop.Theme.Panda');
					}*/
					if (!count($errors)) {
			...
			elseif ($delete && $id_st_wishlist) {
				...
				if ($this->context->customer->isLogged() && $this->context->customer->id == $wishlist->id_customer && Validate::isLoadedObject($wishlist)) {
				    ...
				    $errors[] = $this->trans('Cannot delete this wishlist', array(), 'Shop.Theme.Panda');
				}	
			}
    ...
    public function ajaxProcessAddProduct()
    {
        ...
        if (StWishListClass::addProduct($id_st_wishlist, $this->context->customer->id, $id_product, $id_product_attribute, $quantity)) {
            die(Tools::jsonEncode(array('success' => 1,
                'message' => $this->trans('Product added to wishlist.', array(), 'Shop.Theme.Panda'),
        ...
        die(Tools::jsonEncode(array('success' => 0,
            'message' => $this->trans('Error occurred when adding products.', array(), 'Shop.Theme.Panda'))));
    }
    ...
    public function ajaxProcessDeleteProduct()
    {
        ...
        if (StWishListClass::removeProduct($id_st_wishlist, $id_product, $id_product_attribute)) {
            die(Tools::jsonEncode(array('success' => 1,
            'message' => $this->trans('Product deleted', array(), 'Shop.Theme.Panda'))));
        }
        die(Tools::jsonEncode(array('success' => 0,
            'message' => $this->trans('Failed to delete this product', array(), 'Shop.Theme.Panda'))));
    }
    ...
    public function ajaxProcessCreateWishlist()
    {
        ...
        if ($name) {
            /*if (StWishListClass::isExistsByNameForUser($name)) {
                die(Tools::jsonEncode(array('success' => 0,
                    'message' => $this->trans('This name is already used by another list', array(), 'Shop.Theme.Panda'))));
            }*/
            $wishlist = new StWishListClass();
    		$wishlist->id_shop = $this->context->shop->id;
    		$wishlist->name = $name;
            ...
            if ($wishlist->id) {
                ...
                die(Tools::jsonEncode(array('success' => 1,
                    ...
                    'message' => $this->trans('Wishlist created', array(), 'Shop.Theme.Panda'))));      
            } 
        }
        die(Tools::jsonEncode(array('success' => 0,
            'message' => $this->trans('Create wishilist failed', array(), 'Shop.Theme.Panda'))));
    }
    ...
    public function ajaxProcessUpdateProduct()
    {
        ...
        if (StWishListClass::updateProduct($id_st_wishlist, $id_product, $id_product_attribute, $quantity)) {
            die(Tools::jsonEncode(array('success' => 1,
                'message' => $this->trans('Quantity updated', array(), 'Shop.Theme.Panda'))));
        }
        die(Tools::jsonEncode(array('success' => 0,
            'message' => $this->trans('Update quantity failed', array(), 'Shop.Theme.Panda'))));
    }
    ...
    public function ajaxProcessSendEmail()
    {
        ...
        foreach($email AS $to) {
            if (!Validate::isEmail(trim($to))) {
                $errors[] = $this->trans('Email Address is not correct:', array(), 'Shop.Theme.Panda').$to;
            }
            $wishlist = StWishListClass::exists($id_st_wishlist, $context->customer->id);
    		if (!$wishlist) {
  		        $errors[] = $this->trans('This wishlist does not exist', array(), 'Shop.Theme.Panda');
    		}
            if (!$errors && !StWishListClass::addEmail($id_st_wishlist, $to)) {
                $errors[] = $this->trans('An error occurred when sending this wishlist.', array(), 'Shop.Theme.Panda');
            }
            if ($errors) {
                die(Tools::jsonEncode(array('success' => 0,
                    'message' => $this->trans('An error occurred:', array(), 'Shop.Theme.Panda').implode('<br/>', $errors))));
            }
        ...
        if (!$errors) {
            die(Tools::jsonEncode(array('success' => 1,
             'message' => $this->trans('This wishlist was sent.', array(), 'Shop.Theme.Panda'))));
        }
        die(Tools::jsonEncode(array('success' => 0,
            'message' => $this->trans('An error occurred when sending this wishlist.', array(), 'Shop.Theme.Panda'))));
    }
    ...
    public function checkLogin()
    {
        if (!$this->context->customer->isLogged())
			die(Tools::jsonEncode(array('success' => 2,
				'message' => $this->trans('You aren not logged in', array(), 'Admin.Theme.Panda'))));
    }
}
