...
class StOwlCarousel extends Module implements WidgetInterface
{
	...
	public function getContent()
	{
			...
			if (!count($error) && $slide_group->validateFields(false) && $slide_group->validateFieldsLang(false))
            {
                ...
                if($slide_group->save())
                {
                    ...
                    if(isset($_POST['savestowlcarouselgroupAndStay']) || Tools::getValue('fr') == 'view')
                    {
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Slideshow', array(), 'Modules.StOwlcarousel.Admin').' '.($id_st_owl_carousel_group ? $this->gettranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                }                    
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred during slideshow', array(), 'Modules.StOwlcarousel.Admin').' '.($id_st_owl_carousel_group ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
		...
		if (isset($_POST['savestowlcarousel']) || isset($_POST['savestowlcarouselAndStay']))
		{
            ...
            if (!Tools::isSubmit('has_image_'.$default_lang) && (!isset($_FILES['image_multi_lang_'.$default_lang]) || empty($_FILES['image_multi_lang_'.$default_lang]['tmp_name'])))
			{
			    ...
			    $error[] = $this->displayError($this->getTranslator()->trans('image is required at least in %1%', array('%1%'=>$defaultLanguage->name), 'Admin.Theme.Panda'));
			}
            else
            {
			    $slide->copyFromPost();
			...
			if (!count($error) && $slide->validateFields(false) && $slide->validateFieldsLang(false))
            {
                ...
                if($slide->save())
                {
                    ...
                    $this->_html .= $this->displayError($this->getTranslator()->trans('an error occurred during slide', array(), 'Modules.StOwlcarousel.Admin').' '.($id_st_owl_carousel ? $this->gettranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->gettranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
            else
                $this->_html .= count($error) ? implode('',$error) : $this->displayError($this->getTranslator()->trans('invalid value for field(s).', array(), 'Admin.Theme.Panda'));
        }
	...
	{        
        ...
        $this->fields_form[2]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Mouse drag:', array(), 'Admin.Theme.Panda'),
                    'name' => 'mouse_drag',
                    'default_value' => 1,
                    'is_bool' => true,
                    'values' => array(
                ...
                array(
					...
					'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.
                        '&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
				),
			),
            'buttons' => array(
                array(
        ...
        $this->fields_form[0]['form']['input'][] = array(
			...
			'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.
                $this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
		);
	...
	protected function initFormSlide($isbanner=0)
	{
		...
		$this->fields_form_slide[0]['form'] = array(
			'legend' => array(
				'title' => $this->getTranslator()->trans('Item', array(), 'Admin.Theme.Panda'),
                'icon' => 'icon-cogs'
			),
			'input' => array(
                array(
                ...
                array(
                    ...
                    'size' => 64,
                    'lang' => true,
                    'desc' => '<strong>'.$this->getTranslator()->trans('If this field is filled in, whole image will become clickable. But if there are any links or buttons in the Caption field, this setting will be ignored.', array(), 'Admin.Theme.Panda').'</strong>',
				),
                array(
					'type' => 'switch',
			...
			'input' => array(
                ...
                array(
					...
					'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_owl_carousel_group='.($slide->id_st_owl_carousel_group?$slide->id_st_owl_carousel_group:$id_st_owl_carousel_group).
                        '&viewstowlcarousel&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
				), 
			),
            'buttons' => array(
                array(
		...
		foreach ($languages as $lang)
        {
            $this->fields_form_slide[0]['form']['input']['image_multi_lang_'.$lang['id_lang']] = array(
                    ...
                    'desc' => $this->getTranslator()->trans('This module would not change image names for seo purpose, so ensure the image you are going to upload has an unique name.', array(), 'Admin.Theme.Panda').'<br/>',
                );
        }
        $this->fields_form_slide[0]['form']['input'][] = array(
			...
			'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_owl_carousel_group='.
                ($slide->id_st_owl_carousel_group?$slide->id_st_owl_carousel_group:$id_st_owl_carousel_group).'&viewstowlcarousel&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i>'.$this->getTranslator()->trans('Back to list', array(), 'Admin.Theme.Panda').'</a>',                  
		);
        if(Validate::isLoadedObject($slide))
        {
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'group&id_st_owl_carousel_group='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTRanslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
	...
	protected function initListSlide()
	{
		...
		$helper->title = $this->getTranslator()->trans('Sliders', array(), 'Admin.Theme.Panda');
		$helper->table = $this->name;
    ...
    {
        return '<li class="divider"></li><li><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&copy'.$this->name.'&id_st_owl_carousel='.(int)$id.'&token='.$token.'"><i class="icon-copy"></i>'.$this->getTRanslator()->trans(' Duplicate ', array(), 'Admin.Theme.Panda').'</a></li>';
    }
