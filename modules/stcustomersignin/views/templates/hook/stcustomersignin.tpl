...
{else}
		...
		{if $userinfo_login}
			<div class="quick_login dropdown_wrap top_bar_item">{strip}
	        	<a href="{$my_account_url}" class="dropdown_tri dropdown_tri_in header_item" aria-haspopup="true" aria-expanded="false" rel="nofollow" title="{l s='Log in to your customer account' d='Shop.Theme.Panda'}">
		            {if $show_user_info_icons}<i class="fto-user icon_btn header_v_align_m {if $show_user_info_icons!=2}fs_lg{else}fs_big{/if} mar_r4"></i>{/if}{if $show_user_info_icons!=2}<span class="header_v_align_m">{l s='Login' d='Shop.Theme.Panda'}</span>{/if}
		            <i class="fto-angle-down arrow_down arrow"></i>
		            <i class="fto-angle-up arrow_up arrow"></i>
	            </a>
	            {/strip}
		        <div class="dropdown_list" aria-labelledby="{l s='Login' d='Shop.Theme.Panda'}">
		...
		{else}
		<a class="login top_bar_item" href="{$my_account_url}" rel="nofollow" title="{l s='Log in to your customer account' d='Shop.Theme.Panda'}"><span class="header_item">{if $show_user_info_icons}<i class="fto-user icon_btn header_v_align_m {if $show_user_info_icons!=2}fs_lg{else}fs_big{/if} mar_r4"></i>{/if}{if $show_user_info_icons!=2}<span class="header_v_align_m">{l s='Login' d='Shop.Theme.Panda'}</span>{/if}</span></a>
		{/if}
{/if}