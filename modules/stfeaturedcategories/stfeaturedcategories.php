...
class StFeaturedCategories extends BaseSlider implements WidgetInterface
{
    ...
    protected function initTabNames()
    {
        $this->_tabs = array(
            ...
            array('id'  => '1,3', 'name' => $this->getTranslator()->trans('Slider on the homepage', array(), 'Admin.Theme.Panda')),
            array('id'  => '2', 'name' => $this->getTranslator()->trans('Hooks', array(), 'Admin.Theme.Panda')),
        );
    }
            ...
            {                
                ...
                if ($category->validateFields(false) && $category->validateFieldsLang(false))
                {
                    ...
                    if($category->save())
                    {
                        ...
                        {
                            $this->clearSliderCache();
                            $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Featured category', array(), 'Admin.Theme.Panda').' '.($id_st_featured_category ? $this->getTranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('added', array(), 'Admin.Theme.Panda')));                            
                                                    
                        }
                    }
                    else
                        $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during Featured category', array(), 'Admin.Theme.Panda').' '.($id_st_featured_category ? $this->getTranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
                }
            }
			else
    ...
    public function initFieldsForm()
    {
        ...
        $fields['home_slider']['image_type']['options']['query']['default'] = array(
            'value' => '',
        ...
        //$fields['home_slider']['grid']['label'] = $this->getTranslator()->trans('How to display categories:', array(), 'Admin.Theme.Panda');
	...
	private function _displayForm()
    {
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                array(
					'type' => 'select',
					'label' => $this->getTranslator()->trans('Category:', array(), 'Admin.Theme.Panda'),
					'name' => 'id_category',
                    'required' => true,
					'options' => array(
						'query' => $category_arr,
