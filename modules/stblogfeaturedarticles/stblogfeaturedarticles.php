...
class StBlogFeaturedArticles extends BaseSlider
{
    ...
    public function getContent()
	{
			...
			if (!count($error) && $blog_slider->validateFields(false) && $blog_slider->validateFieldsLang(false))
            {
                ...
                if($blog_slider->save())
                {
                    ...
                    if(isset($_POST['save'.$this->name.'AndStay']))
                        ...
                        $this->_html .= $this->displayConfirmation($this->getTranslator()->trans('Setting', array(), 'Admin.Theme.Panda').' '.($id_st_blog_slider ? $this->getTranslator()->trans('updated', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('added', array(), 'Admin.Theme.Panda')));
                        
                }
                else
                    $this->_html .= $this->displayError($this->getTranslator()->trans('An error occurred during ', array(), 'Modules.StBlog.Admin').' '.($id_st_blog_slider ? $this->getTranslator()->trans('updating', array(), 'Admin.Theme.Panda') : $this->getTranslator()->trans('creation', array(), 'Admin.Theme.Panda')));
            }
			else
		...
		if (Tools::isSubmit('update'.$this->name) || Tools::isSubmit('add'.$this->name))
		{
            ...
            $this->_tabs = array(
                '0' => array('id'  => '0', 'name' => $this->getTranslator()->trans('General settings', array(), 'Admin.Theme.Panda')),
                '1,5' => array('id'  => '1,5', 'name' => $this->getTranslator()->trans('Other settings', array(), 'Admin.Theme.Panda')),
                '2' => array('id'  => '2', 'name' => $this->getTranslator()->trans('Homepage', array(), 'Admin.Theme.Panda')),
                '3' => array('id'  => '3', 'name' => $this->getTranslator()->trans('Left or right column', array(), 'Admin.Theme.Panda')),
                '4' => array('id'  => '4', 'name' => $this->getTranslator()->trans('Footer', array(), 'Admin.Theme.Panda')),
            );
	...
	protected function initForm()
	{
        ...
        $fields['home_slider']['grid']['values'] = array_merge($fields['home_slider']['grid']['values'], array(
            ...
            array(
                'id' => 'grid_zuoyou',
                'value' => 4,
                'label' => $this->getTranslator()->trans('Meida image', array(), 'Admin.Theme.Panda')),
            )
        );
        ...
        $fields['home_slider']['display_sd']['label'] = $this->getTranslator()->trans('Display article short description:', array(), 'Admin.Theme.Panda');
        $fields['home_slider']['spacing_between']['desc'][0] = $this->getTranslator()->trans('Distance between articles.', array(), 'Admin.Theme.Panda');
        ...
        $fields['footer']['nbr_fot']['label'] = $this->getTranslator()->trans('Define the number of articles to be displayed:', array(), 'Admin.Theme.Panda');
	...
	protected function initList()
	{
		...
		$this->fields_list = array(
			...
			'id_st_blog_category' => array(
				'title' => $this->getTranslator()->trans('Blog category', array(), 'Admin.Theme.Panda'),
				'width' => 140,
				'type' => 'text',
    ...
    {        
        ...
        else
        {
            ...
            foreach($result as $k=>&$v)
            {
                ...
                if ($this->type == 2) {
                    $v['link'] = '';
                    $v['name' ] = $this->getTranslator()->trans('Recent articles', array(), 'Modules.StBlog.Admin');
                    // $v['description' ] = '';
