    ...
    {block name='new_customers_container'}
      <div class="col-lg-{12-$auth_login_width}">
        <section id="create_account_block" class="from_blcok block login_form_block">
          {if $sttheme.auth_heading_align!=3}<h3 class="page_heading login_form_heading {if $sttheme.auth_heading_align==1} text-2 {elseif $sttheme.auth_heading_align==2} text-3 {else} text-1 {/if}">{l s='New customers?' d='Shop.Theme.Panda'}</h3>{/if}
          <div class="form_content">
          <div class="form_content_inner">
              <div class="p-b-1">{l s='It\'s quick and easy to create an account to shop faster and save your order to account.' d='Shop.Theme.Panda'}</div>
          </div>
          </div>
    ...
    {block name='login_form_container'}
      <div class="{if $sttheme.auth_layout} col-lg-{$auth_login_width} {else} col-lg-{$auth_login_width} {if $layout=='layouts/layout-full-width.tpl'}offset-lg-{floor((12-$auth_login_width)/2)}{/if} {/if}">{*to do looking for a better way to tell if has columns*}
        <section id="login_form_block" class="from_blcok block login_form_block {if !$sttheme.auth_layout} one_column_login {/if}">
          {if $sttheme.auth_heading_align!=3}<h3 class="page_heading {if $sttheme.auth_heading_align==1} text-2 {elseif $sttheme.auth_heading_align==2} text-3 {else} text-1 {/if}">{l s='Registered customers' d='Shop.Theme.Panda'}</h3>{/if}
          {render file='customer/_partials/login-form.tpl' ui=$login_form}
          {if !$sttheme.auth_layout}
          <div class="form_content_inner text-center p-t-1">
            <h6 class="fs_lg heading_color heading_font">{l s='New customers?' d='Shop.Theme.Panda'}</h6>
            <div class="p-b-1">{l s='It\'s quick and easy to create an account to shop faster and save your order to account.' d='Shop.Theme.Panda'}</div>
          </div>
