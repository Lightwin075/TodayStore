{*The same code in the product.js, using js to add images fast, but not standard*}
              <div class="swiper-slide {if $image.id_image == $product.cover.id_image} clicked_thumb {/if}">
                <div class="pro_gallery_thumb_box general_border {if isset($curr_combination_thumb) && $curr_combination_thumb} curr_combination_thumb {/if}">
                  <img
                      class="pro_gallery_thumb {if !$sttheme.is_ajax}swiper-lazy{/if}"
                      {if $sttheme.product_thumbnails!=3 && !$sttheme.is_ajax}data-{/if}src="{$image.bySize.{$sttheme.thumb_image_type}.url}"
                      {if $sttheme.retina && isset($image.bySize.{$sttheme.thumb_image_type|cat:'_2x'}.url)} {if $sttheme.product_thumbnails!=3 && !$sttheme.is_ajax}data-{/if}srcset="{$image.bySize.{$sttheme.thumb_image_type|cat:'_2x'}.url} 2x" {/if}
